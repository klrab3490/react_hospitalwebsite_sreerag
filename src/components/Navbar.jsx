...
const Navbar = () => {
  ...
  return (
    <div>
      <nav className={`bg-gray-800 px-2 sm:px-4 md:px-6 w-full h-24 flex items-center ${location.pathname !== '/admin' ? 'justify-between' : 'justify-center' }`}>
        {/* Navbar brand centered */}
        <h1 className="text-white text-4xl font-bold">ABC Hospital</h1>

        {/* Navbar toggle button */}
        <button className={`lg:hidden text-white ${location.pathname !== '/admin' ? '' : 'hidden' }`} onClick={() => setIsOpen(!isOpen)}>
      ...
      {isOpen && location.pathname !== '/admin' && (
